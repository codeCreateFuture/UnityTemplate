	...
	public class ResMgr : MonoBehaviour
	{
		...
		public static ResMgr Instance
		{
			get
			{
				if (null == _Instance)
				{
					GameObject go = GameObject.Find(typeof(ResMgr).Name);
					if (null == go)
					{
						go = new GameObject(typeof(ResMgr).Name);
						// DontDestroyOnLoad(go);
					}
					_Instance = go.GetComponent<ResMgr>();

					_Instance = (_Instance == null) ? go.AddComponent<ResMgr>() : _Instance;
