...
/// 常用通用提示框
/// </summary>
public class TextPopHint : MonoBehaviour {

	public static TextPopHint Instance;
	/// <summary>
	/// 过滤文本
	/// </summary>
	...
	public void ShowHint(string msg,float showTime=2f)
	{
	...
	/// 刷新提示框
	/// </summary>
	void UpdateBox(float showTime = 2f)
	{
		//如果当前没有消息，则直接返回
		if (Message.Count == 0)
			return;
		...
		if (Cache.Count == 0)
		{
		   ...
		   // go = GameApp.Instance.ResourcesManagerScript.LoadInstantiateGameObject(path, transform, Vector3.zero);
			string path = "hintitem";
			Debug.Log(path);//Prefab/Item/hintitem
			go =Instantiate( Resources.Load<GameObject>(path),transform);
	...
	/// <param name="go"></param>
	/// <returns></returns>
	IEnumerator CloseBox(GameObject go, string hint, float showTime = 2f)
	{
		//等待2.0f秒钟执行下面的语句
