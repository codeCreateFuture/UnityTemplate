...
public	class NgObject
{
	...
	public static GameObject CreateGameObject(Transform parent, string name)
	{
		...
		{	// 원본 transform을 유지 시켜주자
			//NcTransformTool	trans	= new NcTransformTool(newChild.transform);    //Lixi鬧姦
			//newChild.transform.parent = parent;                                     //Lixi鬧姦
			//trans.CopyToLocalTransform(newChild.transform);                         //Lixi鬧姦
		}
		return newChild;
	}
