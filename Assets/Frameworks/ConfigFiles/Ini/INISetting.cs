...
/*ini文件 config.ini
 ...
 [gateIpOrDN]
gateIpOrDN=116.62.8.17333

[gatePort]
gatePort=5014

[videoIpOrDN]
videoIpOrDN=116.62.8.177

*/

/*用法
 INISetting.GetString("gatePort", INISetting.SectionName.gatePort, "5014");
 */
...
public static class INISetting
    {
        private static string
            mINIFileName =
                Application.dataPath + "/../config.ini"; //Application.dataPath Editor模式是Assets目录路径,打包后是*_Data目录路径

        public enum SectionName
        {
            sectionName, //示例节名,需要新节名,往下配置就行了
            gateIpOrDN, //gate服务器ip或者域名
            videoIpOrDN, //video服务器ip或者域名
            downloadIpOrDN, //download服务器ip或者域名
            gatePort, //gate服务器端口
            videoPort, //video端口
            downloadPort, //download端口
        }

        public static bool GetBool(string Key, SectionName section = SectionName.sectionName, bool defaultValue = false)
        {
        ...
        /// 获取字符串类型的键值
        /// </summary>
        /// <param name="Key">传入键的名字,字符串类型,必传</param>
        /// <param name="section">传入节的名字,可选,不传时默认传sectionName</param>
        /// <param name="defaultValue">传入默认值,当传入的键不存在时,给新传的键赋默认值(有键值时传了不会生效)</param>
        /// <returns></returns>
        public static string GetString(string Key, SectionName section = SectionName.sectionName,
            string defaultValue = "")
        {
        ...
        public static float GetFloat(string Key, SectionName section = SectionName.sectionName, float defaultValue = 0)
        {
        ...
        public static void WriteString(string Key, SectionName section = SectionName.sectionName,
            string defaultValue = "")
        {
            string sectionName = section.ToString();
            Action<INIParser> action =
                (ini) => { ini.WriteValue(sectionName, Key, defaultValue); };
            OpenAndReadINI(mINIFileName, action);
        }

        public static void WriteBool(string Key, SectionName section = SectionName.sectionName,
        ...
        public static void WriteFloat(string Key, SectionName section = SectionName.sectionName, float defaultValue = 0)
        {
            string sectionName = section.ToString();
            Action<INIParser> action =
                (ini) => { ini.WriteValue(sectionName, Key, defaultValue); };
        ...
        public static int GetInt(string Key, SectionName section = SectionName.sectionName, int defaultValue = 0)
        {
